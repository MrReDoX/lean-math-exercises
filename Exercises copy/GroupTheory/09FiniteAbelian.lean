...
Finite abelian groups can have distinct structures even when they have the same cardinality.
The examples below use cyclicity, element orders, and the exponent to distinguish the three
standard groups of order eight.
...
Basic tests for cyclicity, exponents, and invariance of element order under an isomorphism.
**Hover** any name (or place the cursor on the `#check` line and read the infoview) to see its
exact statement.
-/
section

-- Exhibiting a cyclic additive group by the order of one generator.
#check @isAddCyclic_of_addOrderOf_eq_card
#check @isAddCyclic_iff_exists_addOrderOf_eq_natCard

-- Relating the exponent to element orders and transporting orders across an equivalence.
#check @AddMonoid.exponent_eq_max'_addOrderOf
#check @AddEquiv.addOrderOf_eq

-- Rewriting an integer scalar multiplication as repeated addition.
...
`Z/4Z` is not additively isomorphic to `Z/2Z × Z/2Z`: every element of the
...
The element `(1, 0)` in `Z/4Z × Z/2Z` has additive order `4`. -/
...
The three familiar abelian group models for groups of order eight all have
...
`Z/8Z` is cyclic: exhibit a generator by checking that `1` has additive
order eight. -/
...
In a finite abelian group, some element realizes the exponent.

Prove without using `AddMonoid.exists_addOrderOf_eq_exponent`. -/
...
Every element of `Z/2Z × Z/2Z × Z/2Z` is killed by two. This is the
...
The three standard abelian groups of order eight are pairwise
